...
'''
def sampleVsPopulation(popDataset, sampleFeature, selectedFeature, allValues, selectedValues, zCriticalValue):
    delimiter = ':'

    records = popDataset #Read records from the population dataset file

    samples = []

    makeSamples(records, sampleFeature,samples)#Make the samples from the population raw dataset

    records = popDataset #Read the records again because for some reason they disappear after making the sample groups
